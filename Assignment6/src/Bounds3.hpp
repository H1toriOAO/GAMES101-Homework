                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    if (!dirIsNeg[0]) {
        std::swap(x_t_max, x_t_min);
    }

    if (!dirIsNeg[1]) {
        std::swap(y_t_min, y_t_max);
    }

    if (!dirIsNeg[2]) {
        std::swap(z_t_min, z_t_max);
    }

    float t_enter = std::max(x_t_min, std::max(y_t_min, z_t_min));
    float t_exit = std::min(x_t_max, std::min(y_t_max, z_t_max));
